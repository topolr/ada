...
class ViewConnector extends BaseView {
	...
	_updateFromParent(parameter) {
		...
		this.context.logger.log(`● PREDIFF CONNECTOR[PARENT] CHANGE`, this._getChangedProps(), `USED`, this._getUsedProps());
		if (isSubModuleChange(this._getChangedProps(), this._getUsedProps())) {
			this.context.logger.log(`> RENDER FROM PARENT`);
			return this._render();
		} else {
			if (!this.isRendered()) {
				this.context.logger.log(`> RENDER FROM PARENT`);
	...
	_updateFromConnect({data = {}, setter}) {
		...
		this[CHANGEPROPS] = collector.getChangedProps();
		this.context.logger.log(`● PREDIFF CONNECTOR[CONNCT] CHANGE`, this._getChangedProps(), `USED`, this._getUsedProps());
		if (isSubModuleChange(this._getChangedProps(), this._getUsedProps())) {
			this.context.logger.log(`> RENDER FROM CONNECT`);
			return this._render();
		}
		return Promise.resolve();
	}
...
class View extends BaseView {
	...
	_updateFromDataSet() {
		if (this.isRendered()) {
			this.context.logger.log(`● PREDIFF DATASET CHANGE`, this._getChangedProps(), `USED`, this._getUsedProps());
			if (isSubModuleChange(this._getChangedProps(), this._getUsedProps())) {
				this.context.logger.log(`> RENDER FROM DATASET`);
				return this._render();
			}
		} else {
			this.context.logger.log(`> RENDER FROM DATASET`);
	...
	_updateForceFromDataSet() {
		this.context.logger.log(`> RENDER FROM DATASET FORCE`);

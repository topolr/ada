...
const generator = {
    getTargetPath(targetPath) {
        ...
        if (targetPath.substring(0, 2) === "./") {
            path = Path.resolve(util.getProjectConfig().sourcePath, targetPath, "./index.js");
        } else {
            path = Path.resolve(util.getProjectConfig().nodeModulePath, targetPath, "./index.js");
    ...
    createView(path, {name, exname}) {
        let className = name[0].toUpperCase() + name.substring(1);
        return [
            {path: Path.resolve(path, "./index.js"), code: this.parseTemplateCode("view", {name, exname, className})},
            {path: Path.resolve(path, "./state.js"), code: this.parseTemplateCode("state", {className})},
            {path: Path.resolve(path, "./style.scss"), code: this.parseTemplateCode("style", {name})},
    ...
    createConnector(path, {name, exname}) {
        let className = name[0].toUpperCase() + name.substring(1);
        return [
            {
                path: Path.resolve(path, "./index.js"),
                code: this.parseTemplateCode("connector", {name, exname, className})
            },
            {path: Path.resolve(path, "./style.scss"), code: this.parseTemplateCode("style", {name})},
            {path: Path.resolve(path, "./template.html"), code: this.parseTemplateCode("template")}
        ].reduce((a, info) => {
    ...
    create(type, modulePath) {
        ...
        let path = Path.resolve(util.getProjectConfig().sourcePath, modulePath);
        if (type === "connector") {
            return this.createConnector(path, {name, exname});
        } else {
            return this.createView(path, {name, exname});
        }
    },
    extend(targetPath, name) {
        ...
        if (file.exist) {
            ...
            if (codeInfo) {
                ...
                if (_servicePath) {
                    ...
                    if (targetServicePath.indexOf("/node_modules") !== -1) {
                        targetServicePath = targetServicePath.substring(util.getProjectConfig().nodeModulePath.length + 1);
                    }
                }

                let basePath = util.getProjectConfig().sourcePath;
                let tasks = [
                ...
                if (_servicePath) {
                    tasks.push({
                        path: Path.resolve(basePath, name, "./index.js"), code: this.parseTemplateCode("extview", {
                            targetClassPath: targetPath,
                    ...
                    tasks.push({
                        path: Path.resolve(basePath, name, "./state.js"), code: this.parseTemplateCode("extstate", {
                    ...
                    if (path.indexOf("/node_modules") !== -1) {
                        targetPath = path.substring(util.getProjectConfig().nodeModulePath.length + 1);
                    } else {
                        targetPath = Path.dirname(Path.relative(Path.resolve(util.getProjectConfig().sourcePath, name), path));
                    }
                    tasks.push({
                        path: Path.resolve(basePath, name, "./index.js"), code: this.parseTemplateCode("extconnector", {
                            targetClassPath: targetPath,
                            name: codeInfo.className,
                            targetClassName,
                            className
    ...
    clone(targetPath, name) {
        ...
        if (file.exist) {
            ...
            if (codeInfo) {
                ...
                let basePath = util.getProjectConfig().sourcePath;
                let tasks = [
                    {path: Path.resolve(basePath, name, "./style.js"), code: new File(_tempPath).transform().read()},
                    {
                        path: Path.resolve(basePath, name, "./template.html"),
                ...
                if (_servicePath) {
                    ...
                    tasks.push({
                        ...
                        code: new File(_servicePath).readSync()
                    });
                } else {
                    tasks.push({
                        path: Path.resolve(basePath, name, "./index.js"),

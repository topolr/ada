...
const generator = {
	getTargetPath(targetPath) {
		...
		if (targetPath.substring(0, 2) === "./") {
			...
			path = Path.resolve(util.getProjectConfig().nmodulePath, targetPath, "./index.js");
		...
		if (file.exist) {
			...
			if (codeInfo) {
				...
				if (_servicePath) {
					...
					if (isNodeModule) {
						targetServicePath = targetServicePath.substring(util.getProjectConfig().nmodulePath.length);
					} else {
						targetServicePath = Path.relative(Path.resolve(basePath, name), targetServicePath);
					}
				}
				if (isNodeModule) {
					targetClassPath = path.substring(util.getProjectConfig().nmodulePath.length);
				} else {
	...
	clone(targetPath, name) {
		...
		if (file.exist) {
			...
			if (codeInfo) {
				...
				let tasks = [
					{path: Path.resolve(basePath, name, "./template.html"), code: new File(_tempPath).transform().read()},
					{
